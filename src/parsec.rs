            ...
            match Event::unpack(
                ...
                UnpackedEvent::New(event) => {
                    ...
                    #[cfg(feature = "malice-detection")]
                    {
                        ...
                        // We never act on accomplice immediately when we spot it as we need follow
                        // up events to be certain.
...
impl<T: NetworkEvent, S: SecretId> Parsec<T, S> {
    ...
    fn detect_accomplice(
        ...
        let event_hash = *self.get_known_event(event)?.hash();
        ...
        if self.detect_accomplice_for_our_accusations(event, starting_index)? {
            let _ = self.candidate_accomplice_accusations.insert((
                event,
                creator.clone(),
                Malice::Unprovable(UnprovableMalice::Accomplice(event_hash)),
                starting_index,
            ));
        }
